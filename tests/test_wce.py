...
def test_wce_xgb_implementation():
    ...
    wce_preds = clip_sigmoid(bst_wce.predict(dvalid, iteration_range = (0, bst_wce.get_booster().best_iteration)))
    preds = bst.predict(dvalid, iteration_range = (0, bst.get_booster().best_iteration))
    print(preds)
    print(wce_preds)
    assert mean_absolute_error(wce_preds, preds) == 0.0